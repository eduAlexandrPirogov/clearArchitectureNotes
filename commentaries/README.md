...
Это наиболее из самых простых вариантов комментирования. Тут стоит отметить, что мы также описываем, что класс делает в рамках системы, а комментарии к методам поясняют, что делает сам класс.

Но теперь возникает важный вопрос: **мы описываем класс в рамках системы "что делает?", как и методы, но класс может делать определенные вещи, которые будут следовать различным дизайнам, хотя, описывая методы, мы следуем единому дизайну класса (если он хорошо спроектирован), но в случае класса, мы следуем нескольким дизайнам.. Как же описывать классы в рамках системы, чтобы он следовал единому дизайну?"

Ответы описаны ниже в выводе.
...
```cpp
/**
* Преобразовывает строковые json raw данные в виде JSON для более удобной работы. -- зачем нужен класс (Система работает с JSON данными)
**/
class Json
{
