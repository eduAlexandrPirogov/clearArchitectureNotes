...
Первый случай: если класс состоит из одного метода, его можно отнести к уже существующему классу в большинстве случаев.
Второй случай: паттерн "Посетитель". Например, имеется контейнер и мы хотели бы получить некие агрегированные
...
Тут можно либо сделать методы Родительского класса File пустыми, либо использовать паттерн Компоновщик. 
...
3.2
Первый случай: использование паттерна Посетитель, когда можно использовать обычный итератор. Был случай, когда в проекте было два интерфейса Notifyable() и isNotifable(), который разделял две иерархии уведомляемых классов, и для кажой иерархии нужен был свой Наблюдатель. Я создал единый итератор, при этом сократив код на 6 классов.
